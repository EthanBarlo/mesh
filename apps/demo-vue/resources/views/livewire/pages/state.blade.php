<div class="space-y-12">
    <x-demo.page-header
        ...
        description="Two ways data flows between PHP and Vue. useEntangle is a writable ref that stays in sync with a Livewire property in both directions, and anything returned from props() is recomputed on the server each render and handed to your component in place — no remount, so local Vue state survives every request." />

    <x-demo.section
        title="Deferred vs live sync"
        description="Both inputs are entangled with a public Livewire property — the only difference is the second argument to useEntangle. Deferred (the default) updates Vue state instantly but batches the value with the next server request; live commits a round-trip on every keystroke. Watch the round-trip counter and the server-side values: the deferred input stays 'behind' until something triggers a request, like the flush button (wire.$commit()) or typing in the live input."
    ...
    <x-demo.section
        ...
        description="The select below is plain Blade on this page's Livewire component, bound with wire:model.live. The Mesh component receives the same property through wire:model + #[Modelable], and its props() recomputes the palette on the server for every change. Mesh patches the new props into the mounted Vue tree without remounting — the elapsed-seconds timer is a plain local ref and keeps ticking while the palette changes around it."
