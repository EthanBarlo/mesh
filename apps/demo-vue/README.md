# Mesh demo (Laravel + Livewire + Vue)

A small Laravel 12 / Livewire 4 app that demonstrates the Mesh package end-to-end:
a Livewire `Counter` whose view is a Vue component mounted by Mesh. It mirrors
`apps/demo-react` page-for-page, using the `@mesh/vue` renderer and composables.
...
composer dev   # serves Laravel (:8010) + queue + logs + Vite together
```

The dev server uses port `8010` so it can run side by side with `apps/demo-react`
on `:8000`.
...
lives here in `apps/demo-vue/`. The repo root must also be present at runtime
...
```bash
docker build -f apps/demo-vue/Dockerfile -t mesh-demo-vue .
docker run --rm \
  -e APP_KEY=base64:$(openssl rand -base64 32) \
  -e APP_URL=http://localhost:8000 \
  -p 8000:8000 mesh-demo-vue
```
...
to `/apps/demo-vue` shrinks the context and the build breaks.
...
Optionally set Watch Paths to `apps/demo-vue/**`, `src/**`, and `resources/**` so
