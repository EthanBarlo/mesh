<div class="space-y-12">
    <x-demo.page-header
        ...
        description="useWire() hands Svelte the full $wire object — server methods become Promises, Livewire's event bus becomes callbacks. No routes, no controllers, no fetch, no JSON plumbing."
    />

    <x-demo.section
        title="Call the server like a function"
        description="wire.$call('method', …args) invokes a public PHP method and resolves with whatever it returns. The textarea is analyzed by PHP; the dice are rolled by PHP. Svelte just awaits."
    ...
    <x-demo.section
        ...
        description="Svelte dispatches mesh.ping onto Livewire's event bus and a completely plain Livewire component toasts it. In the other direction, a Blade button dispatches page.ping and the island's PHP class catches it with #[On], feeding the count back through props()."
    ...
    >
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch">
            {{-- Svelte → Livewire: the island dispatches, the plain component listens. --}}
            <div class="space-y-4">
                <p class="text-xs font-medium uppercase tracking-widest text-zinc-500">
                    Svelte island <span class="text-rose-400">&rarr;</span> plain Livewire
                </p>
                <mesh:wire.event-bridge />
            </div>

            {{-- Livewire → Svelte: a Blade button dispatches, the island's PHP class listens. --}}
            <div class="space-y-4">
                <p class="text-xs font-medium uppercase tracking-widest text-zinc-500">
                ...
                <div class="p-5 rounded-xl bg-white/[0.02] border border-white/5">
                    ...
                    <p class="mt-2 text-sm text-zinc-400 leading-relaxed">
                        ...
                        dispatches <code class="font-mono text-zinc-300 text-xs">page.ping</code>, which the Svelte island's PHP class catches.
                    </p>
                    <button
                        type="button"
    ...
    <x-demo.section
        ...
        description="The server owns the value; Svelte reacts. An interval schedules wire.$call('tick') every two seconds — PHP random-walks the price — and wire.$watch('price', cb) streams each change into local Svelte state for the sparkline."
