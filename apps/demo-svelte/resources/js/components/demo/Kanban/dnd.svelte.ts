 ...
 *    starts, so every island (separate Svelte apps!) can light up its
 ...
 */
export function useKanbanDrag(): { readonly value: DragPayload | null } {
    let drag = $state.raw<DragPayload | null>(null);

    $effect(() => {
        const onStart = (event: Event) => {
            drag = (event as CustomEvent<DragPayload>).detail;
        };
        const onEnd = () => {
            drag = null;
        };
